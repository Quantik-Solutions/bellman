...
impl<E: Engine> Transpiler<E> {
  ...
  fn enforce_lc_as_gates(
    ...
    let (contains_constant, num_linear_terms) =
      num_unique_values::<E, Self>(&lc, &mut self.scratch);
    ...
    if num_linear_terms <= 3 {
      let (mut a_coef, mut b_coef, mut c_coef) =
        rewrite_lc_into_single_enforcement_gate(&lc, self, &mut (self.scratch.clone()));
      ...
      let hint =
        TranspilationVariant::<E>::IntoSingleAdditionGate((a_coef, b_coef, c_coef, constant_coeff));

      return hint;
    } else {
      let (first_gate, mut other_coefs) =
        rewrite_lc_into_multiple_enforcement_gates(&lc, self, &mut (self.scratch.clone()));
      ...
      let hint = TranspilationVariant::<E>::IntoMultipleAdditionGates(
        (a_coef, b_coef, c_coef, constant_coeff),
        other_coefs,
      );
  ...
  fn rewrite_lc(
    ...
    multiplier: E::Fr,
    free_term_constant: E::Fr,
  ) -> (Variable, TranspilationVariant<E>) {
    let zero_fr = E::Fr::zero();
    let one_fr = E::Fr::one();
    ...
    if num_linear_terms == 1 && !contains_constant && free_term_constant == zero_fr {
      ...
      let (new_var, (mut a_coef, mut b_coef, mut c_coef, mut constant_coeff)) =
        rewrite_lc_into_single_addition_gate(&lc, self, &mut (self.scratch.clone()));
      // we've made a sinlge addition gate, but we may need to scale it (my multipler)
      // and also account for a RHS in a form of = constant_coeff
      ...
      let (new_var, first_gate, mut other_coefs) =
        rewrite_lc_into_multiple_addition_gates(&lc, self, &mut (self.scratch.clone()));
...
impl<E: Engine> crate::ConstraintSystem<E> for Transpiler<E> {
    ...
    LC: FnOnce(crate::LinearCombination<E>) -> crate::LinearCombination<E>,
  {
    ...
    let a = deduplicate_stable::<E, Self>(
      a(crate::LinearCombination::zero()),
      &mut self.deduplication_scratch,
    );
    let b = deduplicate_stable::<E, Self>(
      b(crate::LinearCombination::zero()),
      &mut self.deduplication_scratch,
    );
    let c = deduplicate_stable::<E, Self>(
    ...
    match (a_is_constant, b_is_constant, c_is_constant) {
      ...
      (true, false, true) | (false, true, true) => {
        ...
        let hint = TranspilationVariant::<E>::MergeLinearCombinations((
      ...
      (false, false, true) => {
        ...
        let (is_quadratic_gate, coeffs) =
          is_quadratic_gate::<E, Self>(&a, &b, &c, &mut self.scratch);
...
) -> ((E::Fr, E::Fr, E::Fr), Vec<E::Fr>) // first rewrite is full, than it's Z + a * X - Y = 0
{
  ...
  let _new_var = cs
    .alloc(|| "allocate addition gate", || unreachable!())
...
impl<'a, E: Engine, CS: PlonkConstraintSystem<E> + 'a> Adaptor<'a, E, CS> {
  ...
  fn make_single_addition_gate(
    ...
    if b_coeff.is_zero() {
      ...
      self.cs.new_gate(
        (a_var, self.cs.get_dummy_variable(), new_var),
        (a_coeff, b_coeff, c_coeff, zero_fr, constant_coeff),
      )?;
  ...
  fn make_chain_of_addition_gates(
    ...
    for (var, _) in &mut it {
      if var != &cs_one {
        ...
        self.cs.new_gate(
          (old_new_var, original_var, new_var),
          (one_fr, hint_coeff, minus_one_fr, zero_fr, zero_fr),
        )?;
      }
    }
    ...
    loop {
      ...
      if var != &cs_one {
        if gates_created != cycle_len - 2 {
          ...
          let hint_coeff = chain_iter.next().expect("there should be a chain coeff");
          let hint_coeff_last = chain_iter
            .next()
            .expect("there should be a last chain coeff");
          let original_var = convert_variable(*var);
          let original_last_var = convert_variable(*last_var);

          self.cs.new_gate(
            (new_var, original_var, original_last_var),
            (one_fr, hint_coeff, hint_coeff_last, zero_fr, zero_fr),
          )?;
          break;
        }
      } else {
    ...
    match hint {
      TranspilationVariant::IntoLinearGate((c0, c1)) => {
        ...
        self
          .cs
          .new_gate(
            (var, dummy_var, dummy_var),
            (c1, zero_fr, zero_fr, zero_fr, c0),
          )
      ...
      TranspilationVariant::IntoQuandaticGate((c0, c1, c2)) => {
        ...
        self
          .cs
          .new_gate((var, var, dummy_var), (c1, zero_fr, zero_fr, c2, c0))
          .expect("must make a gate");
      }
      TranspilationVariant::TransformLc(boxed_hints) => {
        ...
        let a_var = match t_a {
          TranspilationVariant::IntoSingleAdditionGate(coeffs) => self
            .make_single_addition_gate(&a, coeffs)
            .expect("must make a gate"),
          TranspilationVariant::IntoMultipleAdditionGates(coeffs, chain) => self
            .make_chain_of_addition_gates(&a, coeffs, chain)
            .expect("must make a gate"),
        ...
        let b_var = match t_b {
          TranspilationVariant::IntoSingleAdditionGate(coeffs) => self
            .make_single_addition_gate(&b, coeffs)
            .expect("must make a gate"),
          TranspilationVariant::IntoMultipleAdditionGates(coeffs, chain) => self
            .make_chain_of_addition_gates(&b, coeffs, chain)
            .expect("must make a gate"),
          TranspilationVariant::LeaveAsSingleVariable(coeff) => {
        ...
        let (c_is_just_a_constant, c_var, mut c_coeff) = match t_c {
          TranspilationVariant::IntoSingleAdditionGate(coeffs) => (
            ...
            Some(
              self
                .make_single_addition_gate(&c, coeffs)
                .expect("must make a gate"),
            ),
            one_fr,
          ),
          TranspilationVariant::IntoMultipleAdditionGates(coeffs, chain) => (
            ...
            Some(
              self
                .make_chain_of_addition_gates(&c, coeffs, chain)
                .expect("must make a gate"),
            ),
            one_fr,
          ),
        ...
        if c_is_just_a_constant {
          ...
          self
            ...
            .new_gate(
              (a_var, b_var, c_var),
              (zero_fr, zero_fr, c_coeff, multiplication_constant, zero_fr),
            )
            .expect("must make a gate");
        }
      }
      ...
      TranspilationVariant::MergeLinearCombinations((merge_variant, coeff, merge_hint)) => {
        ...
        match h {
          TranspilationVariant::IntoSingleAdditionGate(coeffs) => {
            self
              .enforce_lc_with_single_addition_gate(lc_into_rewriting, coeffs)
              .expect("must make a gate");
          }
          TranspilationVariant::IntoMultipleAdditionGates(coeffs, chain) => {
            self
              .enforce_lc_using_chain_of_addition_gates(lc_into_rewriting, coeffs, chain)
              .expect("must make a gate");
          }
          // TranspilationVariant::LeaveAsSingleVariable(coeff) => {
