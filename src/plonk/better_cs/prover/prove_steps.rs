...
pub(crate) fn get_precomputed_permutation_poly_lde_for_index<
  'a,
  E: Engine,
  ...
  if let Some(prec) = setup_precomputations {
    ...
    match &*precomputed_omegas {
      PrecomputedOmegas::None => {
        *precomputed_omegas =
          PrecomputedOmegas::Owned(CP::new_for_domain_size(domain_size), E::Fr::one());
      }
      _ => {}
    }
...
pub(crate) fn get_precomputed_selector_lde_for_index<
  'a,
  E: Engine,
  CP: CTPrecomputations<E::Fr>,
>(
...
pub(crate) fn get_precomputed_next_step_selector_lde_for_index<
  'a,
  E: Engine,
  CP: CTPrecomputations<E::Fr>,
>(
...
pub(crate) fn get_precomputed_inverse_divisor<'a, E: Engine>(
  ...
  if let Some(prec) = setup_precomputations {
    ...
    return Ok(PrecomputationsForPolynomial::Owned(
...
impl<E: Engine> ProverAssembly4WithNextStep<E> {
  ...
  > {
    ...
    match &*precomputed_omegas_inv {
      PrecomputedOmegas::None => {
        *precomputed_omegas_inv =
          PrecomputedOmegas::Owned(CPI::new_for_domain_size(required_domain_size), E::Fr::one());
      }
      _ => {}
    }
    ...
    for wire_poly in full_assignments.iter() {
      ...
      let as_coeffs = as_poly.ifft_using_bitreversed_ntt(
        &worker,
  ...
  > {
    ...
    let z_in_monomial_form =
      z.ifft_using_bitreversed_ntt(&worker, precomputed_omegas_inv.as_ref(), &E::Fr::one())?;
  ...
  > {
    ...
    match &*precomputed_omegas {
      PrecomputedOmegas::None => {
        *precomputed_omegas =
          PrecomputedOmegas::Owned(CP::new_for_domain_size(required_domain_size), E::Fr::one());
      }
      _ => {}
    }
