...
pub fn write_polynomial<F: PrimeField, P: PolynomialForm, W: Write>(
  p: &Polynomial<F, P>,
  mut writer: W,
) -> std::io::Result<()> {
...
pub fn read_polynomial_coeffs<F: PrimeField, R: Read>(
  mut reader: R,
) -> std::io::Result<Polynomial<F, Coefficients>> {
...
pub fn read_polynomial_values_unpadded<F: PrimeField, R: Read>(
  mut reader: R,
) -> std::io::Result<Polynomial<F, Values>> {
  let num_values = reader.read_u64::<BigEndian>()?;
...
impl<E: Engine, P: PlonkConstraintSystemParams<E>> SetupPolynomialsPrecomputations<E, P> {
  pub fn from_setup_and_precomputations<CP: CTPrecomputations<E::Fr>>(
    ...
    let mut new = Self {
      ...
      inverse_divisor_on_coset_of_size_4n_bitreversed: Polynomial::from_values(vec![E::Fr::one()])
        .unwrap(),
