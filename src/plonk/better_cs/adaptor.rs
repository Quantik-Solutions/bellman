    ...
    LC: FnOnce(crate::LinearCombination<E>) -> crate::LinearCombination<E>,
  {
    ...
    match (a_is_constant, b_is_constant, c_is_constant) {
      ...
      (true, false, true) | (false, true, true) => {
        ...
        let (_, _, hint) =
          enforce_lc_as_gates(self, lc, multiplier, free_constant_term, false, &mut space)
      ...
      (true, false, false) | (false, true, false) => {
        ...
        if multiplier == zero_fr {
          ...
          let (_, _, hint_c) = enforce_lc_as_gates(self, c_lc, one_fr, zero_fr, false, &mut space)
        ...
        let a_var = match t_a {
          ...
          | hint @ LcTransformationVariant::IntoMultipleGates => {
            let (new_a_var, a_coeff, _variant) =
              enforce_lc_as_gates(self.cs, a_lc, one_fr, a_constant_term, true, &mut space)
        ...
        let b_var = match t_b {
          ...
          | hint @ LcTransformationVariant::IntoMultipleGates => {
            let (new_b_var, b_coeff, _variant) =
              enforce_lc_as_gates(self.cs, b_lc, one_fr, b_constant_term, true, &mut space)
        ...
        let (c_is_just_a_constant, c_var) = match t_c {
          ...
          | hint @ LcTransformationVariant::IntoMultipleGates => {
            let (new_c_var, c_coeff, _variant) =
              enforce_lc_as_gates(self.cs, c_lc, one_fr, c_constant_term, true, &mut space)
...
fn transpile_xor_using_new_adaptor() {
  ...
  let is_valid =
    verify::<Bn256, PlonkCsWidth4WithNextStepParams, Transcr>(&proof, &verification_key).unwrap();
  ...
  let mut proof_writer =
    std::io::BufWriter::with_capacity(1 << 24, std::fs::File::create("./xor_proof.proof").unwrap());
...
fn transpile_xor_and_prove_with_no_precomputations() {
  ...
  let proof =
    super::super::prove_by_steps::<_, _, Transcr>(c, &hints, &setup, None, &crs_mons).unwrap();
