    ...
    match (a_is_constant, b_is_constant, c_is_constant) {
      ...
      (false, false, true) => {
        ...
        let (_new_a_var, _, hint_a, _) =
          enforce_lc_as_gates(self, a_lc, one_fr, a_constant_term, true, false, &mut space)
            .expect("must allocate A LC as gates for constraint like LC * LC = c1");

        let (_new_b_var, _, hint_b, _) =
          enforce_lc_as_gates(self, b_lc, one_fr, b_constant_term, true, false, &mut space)
      ...
      (false, false, false) => {
        ...
        let (_new_c_var, _, hint_c, can_use_fma) =
          enforce_lc_as_gates(self, c_lc, one_fr, c_constant_term, true, true, &mut space)
