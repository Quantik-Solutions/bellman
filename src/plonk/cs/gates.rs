...
impl<F: PrimeField> std::fmt::Debug for Coeff<F> {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    match self {
      Coeff::Zero => write!(f, "Coeff 0x0"),
      Coeff::One => write!(f, "Coeff 0x1"),
      Coeff::NegativeOne => write!(f, "Coeff -0x1"),
      Coeff::Full(c) => write!(f, "Coeff {:?}", c),
    }
  }
}
...
impl<F: PrimeField> Gate<F> {
  ...
  pub(crate) fn new_enforce_zero_gate(
    variables: (Variable, Variable, Variable),
    coeffs: (F, F, F),
  ) -> Self {
    let (a_coeff, b_coeff, c_coeff) = coeffs;
  ...
  pub(crate) fn new_enforce_constant_gate(
    variable: Variable,
    constant: Option<F>,
