...
impl<E: Engine> ConstraintSystem<E> for ProvingAssembly<E> {
  ...
  fn enforce_constant(
    ...
    let gate =
      Gate::<E::Fr>::new_enforce_constant_gate(variable, Some(constant), self.dummy_variable());
    self.aux_gates.push(gate);
    self.n += 1;
...
impl<E: Engine> ProvingAssembly<E> {
  pub(crate) fn new() -> Self {
    ...
    tmp
      .enforce_constant(zero, E::Fr::zero())
      .expect("should have no issues");
  ...
  fn calculate_inverse_vanishing_polynomial_in_a_coset(
    ...
    let mut numerator =
      Polynomial::<E::Fr, Values>::from_values(vec![multiplicative_generator; poly_size])?;
  ...
  E::Fr: PartialTwoBitReductionField,
{
  ...
  >(
    ...
    let a_commitment_data =
      Self::commit_single_poly(&a_poly, omegas_bitreversed, &params, &worker)?;
    let b_commitment_data =
      Self::commit_single_poly(&b_poly, omegas_bitreversed, &params, &worker)?;
    let c_commitment_data =
      Self::commit_single_poly(&c_poly, omegas_bitreversed, &params, &worker)?;
    ...
    let z_1 = z_1.ifft_using_bitreversed_ntt_with_partial_reduction(
      &worker,
      omegas_inv_bitreversed,
      &E::Fr::one(),
    )?;
    let z_2 = z_2.ifft_using_bitreversed_ntt_with_partial_reduction(
      &worker,
      omegas_inv_bitreversed,
      &E::Fr::one(),
    )?;
    ...
    let a_coset_lde_bitreversed = a_commitment_data
      .poly
      .clone_subset_assuming_bitreversed(partition_factor)?;
    let b_coset_lde_bitreversed = b_commitment_data
      .poly
      .clone_subset_assuming_bitreversed(partition_factor)?;
    let c_coset_lde_bitreversed = c_commitment_data
      .poly
      .clone_subset_assuming_bitreversed(partition_factor)?;

    let q_l_coset_lde_bitreversed = setup_precomp
      .q_l_aux
      .poly
      .clone_subset_assuming_bitreversed(partition_factor)?;
    let q_r_coset_lde_bitreversed = setup_precomp
      .q_r_aux
      .poly
      .clone_subset_assuming_bitreversed(partition_factor)?;
    let q_o_coset_lde_bitreversed = setup_precomp
      .q_o_aux
      .poly
      .clone_subset_assuming_bitreversed(partition_factor)?;
    let q_m_coset_lde_bitreversed = setup_precomp
      .q_m_aux
      .poly
      .clone_subset_assuming_bitreversed(partition_factor)?;
    let q_c_coset_lde_bitreversed = setup_precomp
      .q_c_aux
      .poly
      .clone_subset_assuming_bitreversed(partition_factor)?;
    let s_id_coset_lde_bitreversed = setup_precomp
      .s_id_aux
      .poly
      .clone_subset_assuming_bitreversed(partition_factor)?;
    let sigma_1_coset_lde_bitreversed = setup_precomp
      .sigma_1_aux
      .poly
      .clone_subset_assuming_bitreversed(partition_factor)?;
    let sigma_2_coset_lde_bitreversed = setup_precomp
      .sigma_2_aux
      .poly
      .clone_subset_assuming_bitreversed(partition_factor)?;
    let sigma_3_coset_lde_bitreversed = setup_precomp
      .sigma_3_aux
      .poly
      .clone_subset_assuming_bitreversed(partition_factor)?;
    ...
    let z_1_coset_lde_bitreversed = z_1_commitment_data
      .poly
      .clone_subset_assuming_bitreversed(partition_factor)?;
    ...
    let z_2_coset_lde_bitreversed = z_2_commitment_data
      .poly
      .clone_subset_assuming_bitreversed(partition_factor)?;
    ...
    let t_poly_high_commitment_data =
      Self::commit_single_poly(&t_poly_high, omegas_bitreversed, &params, &worker)?;
    let t_poly_mid_commitment_data =
      Self::commit_single_poly(&t_poly_mid, omegas_bitreversed, &params, &worker)?;
    let t_poly_low_commitment_data =
      Self::commit_single_poly(&t_poly_low, omegas_bitreversed, &params, &worker)?;
    ...
    let _ = Self::multiopening(
      vec![
        witness_opening_request_at_z,
        witness_opening_request_at_z_omega,
      ],
...
mod test {
  ...
  fn test_bench_redshift() {
    ...
    for (size, coset_schedule) in sizes.into_iter().zip(coset_schedules.into_iter()) {
      ...
      let omegas_inv_bitreversed_for_fri =
        <CosetOmegasInvBitreversed<Fr> as FriPrecomputations<Fr>>::new_for_domain_size(
          size.next_power_of_two() * rate,
        );
      ...
      let _ = prover
        .prove_with_setup_precomputed::<_, _, _, Transcr>(
          &setup_precomp,
          &params,
          &worker,
          &omegas_bitreversed,
  ...
  fn test_ifft_using_ntt() {
    ...
    for poly_size in poly_sizes.clone().into_iter() {
      ...
      let omegas_inv_bitreversed =
        <OmegasInvBitreversed<Fr> as CTPrecomputations<Fr>>::new_for_domain_size(
          (poly_size as usize).next_power_of_two(),
        );
      let ntt_result = poly
        .clone()
        .ifft_using_bitreversed_ntt(&worker, &omegas_inv_bitreversed, &Fr::one())
        .unwrap();
    ...
    for poly_size in poly_sizes.into_iter() {
      let coeffs = (0..poly_size).map(|_| Fr::rand(rng)).collect::<Vec<_>>();
      let poly = Polynomial::<Fr, _>::from_values(coeffs).unwrap();
      let naive_result = poly
        .clone()
        .icoset_fft_for_generator(&worker, &Fr::multiplicative_generator());
      let omegas_inv_bitreversed =
        <OmegasInvBitreversed<Fr> as CTPrecomputations<Fr>>::new_for_domain_size(
          (poly_size as usize).next_power_of_two(),
        );
