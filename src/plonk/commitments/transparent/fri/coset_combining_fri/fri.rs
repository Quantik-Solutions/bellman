...
impl<F: PrimeField> FriIop<F> for CosetCombiningFriIop<F> {
  ...
  fn proof_from_lde<
    P: Prng<F, Input = <Self::IopType as IopInstance<F>>::Commitment>,
...
impl<F: PrimeField> CosetCombiningFriIop<F> {
  pub fn proof_from_lde_by_values<
    P: Prng<F, Input = <<Self as FriIop<F>>::IopType as IopInstance<F>>::Commitment>,
    C: FriPrecomputations<F>,
  >(
    lde_values: &Polynomial<F, Values>,
    lde_factor: usize,
          ...
          scope.spawn(move |_| {
            ...
            for (j, v) in v.iter_mut().enumerate() {
              ...
              for (wrapping_step, challenge) in next_domain_challenges.iter().enumerate() {
                ...
                for (pair_idx, (pair, o)) in inputs
                  .chunks(2)
                  .zip(next_level_values[..expected_next_level_values].iter_mut())
                  .enumerate()
                {
...
mod test {
  ...
  fn test_bench_fri_with_coset_combining() {
    ...
    let eval_result = poly
      .bitreversed_lde_using_bitreversed_ntt(&worker, 16, &precomp, &coset_factor)
      .unwrap();
    println!(
    ...
    let fri_precomp =
      <OmegasInvBitreversed<Fr> as FriPrecomputations<Fr>>::new_for_domain_size(eval_result.size());
