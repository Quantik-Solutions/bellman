...
impl<E: Engine> IndexPrecomputations<E> {
  pub fn new(params: &IndexedSetup<E>, worker: &Worker) -> Result<Self, SynthesisError> {
    ...
    let r_x_x_values_over_h =
      eval_unnormalized_bivariate_lagrange_poly_over_diaginal(domain_h.size, &domain_h, &worker);
    let r_x_x_values_over_h = Polynomial::from_values(r_x_x_values_over_h)?;

    let a_row_over_2k_coset: Polynomial<E::Fr, Values> =
      params.a_row_poly.clone().coset_lde(&worker, 2)?;
    let b_row_over_2k_coset: Polynomial<E::Fr, Values> =
      params.b_row_poly.clone().coset_lde(&worker, 2)?;
    let c_row_over_2k_coset: Polynomial<E::Fr, Values> =
      params.c_row_poly.clone().coset_lde(&worker, 2)?;
    let a_col_over_2k_coset: Polynomial<E::Fr, Values> =
      params.a_col_poly.clone().coset_lde(&worker, 2)?;
    let b_col_over_2k_coset: Polynomial<E::Fr, Values> =
      params.b_col_poly.clone().coset_lde(&worker, 2)?;
    let c_col_over_2k_coset: Polynomial<E::Fr, Values> =
      params.c_col_poly.clone().coset_lde(&worker, 2)?;
    let a_val_over_2k_coset: Polynomial<E::Fr, Values> =
      params.a_matrix_poly.clone().coset_lde(&worker, 2)?;
    let b_val_over_2k_coset: Polynomial<E::Fr, Values> =
      params.b_matrix_poly.clone().coset_lde(&worker, 2)?;
    let c_val_over_2k_coset: Polynomial<E::Fr, Values> =
      params.c_matrix_poly.clone().coset_lde(&worker, 2)?;
...
impl<E: Engine> PrecomputedBases<E> {
  pub fn new_42_for_index(params: &IndexedSetup<E>, worker: &Worker) -> Self {
    ...
    let crs_values_on_h =
      Crs::<E, CrsForLagrangeForm>::from_powers(&monomial, params.domain_h_size, &worker);
    println!("Done making lagrange bases on H");

    let crs_values_on_k =
      Crs::<E, CrsForLagrangeForm>::from_powers(&monomial, params.domain_k_size, &worker);
...
impl<E: Engine> PreparedProver<E> {
  pub fn create_proof(
    ...
    let h_poly_values_on_h = {
      ...
      let z_in_coset =
        evaluate_vanishing_for_size(&E::Fr::multiplicative_generator(), domain_h.size);
    ...
    let witness_commitment =
      commit_using_values(&witness_values_on_h, &crs.crs_values_on_h, &worker)?;
    ...
    fn construct_r_m_from_matrix<F: PrimeField>(
      ...
      worker.scope(col_indexes.len(), |scope, chunk_size| {
        for (chunk_id, ((subres, row_chunk), col_chunk)) in subresults
          .chunks_mut(1)
    ...
    let tmp = r_alpha_x_values_over_h
      .clone()
      .ifft(&worker)
      .lde(&worker, 2)
      .unwrap();
    ...
    let mut proper_t_1_values_on_2h = r_m_alpha_x_sum
      .clone()
      .ifft(&worker)
      .lde(&worker, 2)
      .unwrap();
    let tmp = witness_values_on_h
      .clone()
      .ifft(&worker)
      .lde(&worker, 2)
      .unwrap();
    ...
    fn calculate_grand_sum_over_subdomain_assuming_natural_ordering_with_normalization<
      F: PrimeField,
    >(
      ...
      let subdomain_size_as_fe =
        F::from_str(&format!("{}", values.size() / 2)).expect("must be a valid element");
      let one_over_size = subdomain_size_as_fe
        .inverse()
      ...
      worker.scope(0, |scope, _| {
        for (chunk_idx, ((g, s_main), s_sub)) in result[(chunk_len + 2)..]
      ...
      assert_eq!(
        alt_total_sum_main, domain_sum_main,
        "sum on main domain must match"
      );
      assert_eq!(
        alt_total_sum_sub, domain_sum_sub,
        "sum on subdomain must match"
      );
    ...
    let mut quotient = proper_q_1_values_on_2h
      .clone()
      .ifft(&worker)
      .coset_fft(&worker);
    let proper_q_1_grand_sum_poly_values_over_2h_coeffs = proper_q_1_grand_sum_poly_values_over_2h
      .clone()
      .ifft(&worker);

    let mut proper_q_1_grand_sum_poly_values_over_2h_coeffs_shifted =
      proper_q_1_grand_sum_poly_values_over_2h_coeffs.clone();
    proper_q_1_grand_sum_poly_values_over_2h_coeffs_shifted
      .distribute_powers(&worker, domain_h.generator);

    quotient.add_assign(
      &worker,
      &proper_q_1_grand_sum_poly_values_over_2h_coeffs.coset_fft(&worker),
    );
    quotient.sub_assign(
      &worker,
      &proper_q_1_grand_sum_poly_values_over_2h_coeffs_shifted.coset_fft(&worker),
    );
    ...
    let (q_1_sum_over_h, q_1_grand_sum_poly_values_over_h) =
      q_1_poly_values_over_h.calculate_grand_sum(&worker)?;
    ...
    let one_over_h_size = size_h_as_fe
      .inverse()
      .ok_or(SynthesisError::DivisionByZero)?;
    ...
    let q_2_sum_commitment =
      commit_using_values(&q_2_grand_sum_over_h, &crs.crs_values_on_h, &worker)?;
    ...
    fn evaluate_bivariate_lagrange_over_row_or_col_poly<F: PrimeField>(
      ...
      let inv_vanishing_at_x = vanishing_at_x
        .inverse()
        .ok_or(SynthesisError::DivisionByZero)?;
      ...
      inverses
        .batch_inversion(&worker)
        .expect("must inverse as there are no zeroes");
    ...
    let (t_a, q_3_a_by_eta_a_grand_sum_over_k) =
      f_3_a_values_over_k_by_eta_a.calculate_grand_sum(&worker)?;
    let (t_b, q_3_b_by_eta_b_grand_sum_over_k) =
      f_3_b_values_over_k_by_eta_b.calculate_grand_sum(&worker)?;
    let (t_c, q_3_c_by_eta_c_grand_sum_over_k) =
      f_3_c_values_over_k_by_eta_c.calculate_grand_sum(&worker)?;
    ...
    let q_3_a_by_eta_a_commitment =
      commit_using_values(&f_3_a_values_over_k_by_eta_a, &crs.crs_values_on_k, &worker)?;
    let q_3_a_by_eta_a_sum_commitment = commit_using_values(
    ...
    let q_3_b_by_eta_b_commitment =
      commit_using_values(&f_3_b_values_over_k_by_eta_b, &crs.crs_values_on_k, &worker)?;
    let q_3_b_by_eta_b_sum_commitment = commit_using_values(
    ...
    let q_3_c_by_eta_c_commitment =
      commit_using_values(&f_3_c_values_over_k_by_eta_c, &crs.crs_values_on_k, &worker)?;
    let q_3_c_by_eta_c_sum_commitment = commit_using_values(
    ...
    let q_3_a_by_eta_a_values_over_2k_coset = q_3_a_by_eta_a_poly_coeffs
      .clone()
      .coset_lde(&worker, lde_factor_for_q_3_check_over_k)?;
    let q_3_b_by_eta_b_values_over_2k_coset = q_3_b_by_eta_b_poly_coeffs
      .clone()
      .coset_lde(&worker, lde_factor_for_q_3_check_over_k)?;
    let q_3_c_by_eta_c_values_over_2k_coset = q_3_c_by_eta_c_poly_coeffs
      .clone()
      .coset_lde(&worker, lde_factor_for_q_3_check_over_k)?;
    ...
    let lde_factor_vals = (params.domain_k_size as usize) * lde_factor_for_q_3_check_over_k
      / params.a_matrix_poly.size();
    ...
    let lde_factor_row_col =
      (params.domain_k_size as usize) * lde_factor_for_q_3_check_over_k / params.a_row_poly.size();
    ...
    let mut f_3_well_formedness_poly_values_over_2k_coset =
      precomputations.a_val_over_2k_coset.clone();
    f_3_well_formedness_poly_values_over_2k_coset.scale(&worker, val_a_total_coeffs);
    f_3_well_formedness_poly_values_over_2k_coset.add_assign_scaled(
      &worker,
      &precomputations.b_val_over_2k_coset,
      &val_b_total_coeffs,
    );
    f_3_well_formedness_poly_values_over_2k_coset.add_assign_scaled(
      &worker,
      &precomputations.c_val_over_2k_coset,
      &val_c_total_coeffs,
    );
    ...
    let vanishing_in_coset_over_k =
      evaluate_vanishing_for_size(&E::Fr::multiplicative_generator(), domain_k.size);
    let vanishing_in_coset_over_k = vanishing_in_coset_over_k
      .inverse()
      .ok_or(SynthesisError::DivisionByZero)?;
    // for other elements x^n - 1 = (generator*omega)^n - 1 = - generator^n - 1 cause omega^2n == 1 on a large domain
    let mut vanishing_in_coset_over_k_shifted =
      E::Fr::multiplicative_generator().pow(&[domain_k.size]);
    vanishing_in_coset_over_k_shifted.negate();
    vanishing_in_coset_over_k_shifted.sub_assign(&E::Fr::one());
    let vanishing_in_coset_over_k_shifted = vanishing_in_coset_over_k_shifted
      .inverse()
      .ok_or(SynthesisError::DivisionByZero)?;

    worker.scope(
      f_3_well_formedness_poly_values_over_2k_coset.size(),
      |scope, chunk_size| {
        for (chunk_id, chunk) in f_3_well_formedness_poly_values_over_2k_coset
          .as_mut()
    ...
    let f_3_well_formedness_baryc_at_beta_3 = f_3_well_formedness_poly_values_over_2k_coset
      .barycentric_over_coset_evaluate_at(&worker, beta_3, &E::Fr::multiplicative_generator())?;

    let (f_3_even_values_on_k, f_3_odd_values_on_k) = f_3_well_formedness_poly_values_over_2k_coset
      .split_into_even_and_odd_assuming_natural_ordering(
        &worker,
        &E::Fr::multiplicative_generator(),
      )?;
    ...
    let f_3_even_commitment =
      commit_using_values(&f_3_even_values_on_k, &crs.crs_values_on_k, &worker)?;
    let f_3_odd_commitment =
      commit_using_values(&f_3_odd_values_on_k, &crs.crs_values_on_k, &worker)?;
    ...
    let coset_offset_inv = E::Fr::multiplicative_generator()
      .inverse()
      .ok_or(SynthesisError::DivisionByZero)?;
    ...
    let f_3_even_eval = f_3_even_values_on_k
      .barycentric_evaluate_at(&worker, beta_3_squared_by_coset_factor_squared)?;
    let f_3_odd_eval = f_3_odd_values_on_k
      .barycentric_evaluate_at(&worker, beta_3_squared_by_coset_factor_squared)?;
    ...
    let q_3_a_by_eta_a_sum_eval =
      q_3_a_by_eta_a_grand_sum_over_k.barycentric_evaluate_at(&worker, beta_3)?;
    let q_3_b_by_eta_b_sum_eval =
      q_3_b_by_eta_b_grand_sum_over_k.barycentric_evaluate_at(&worker, beta_3)?;
    let q_3_c_by_eta_c_sum_eval =
      q_3_c_by_eta_c_grand_sum_over_k.barycentric_evaluate_at(&worker, beta_3)?;
    let q_3_a_by_eta_a_sum_eval_shifted =
      q_3_a_by_eta_a_grand_sum_over_k.barycentric_evaluate_at(&worker, beta_3_by_omega)?;
    let q_3_b_by_eta_b_sum_eval_shifted =
      q_3_b_by_eta_b_grand_sum_over_k.barycentric_evaluate_at(&worker, beta_3_by_omega)?;
    let q_3_c_by_eta_c_sum_eval_shifted =
      q_3_c_by_eta_c_grand_sum_over_k.barycentric_evaluate_at(&worker, beta_3_by_omega)?;
    ...
    let polys_for_opening_for_domain_k_at_beta_3_by_gen =
      vec![f_3_even_values_on_k, f_3_odd_values_on_k];
    ...
    let q_2_sum_eval_at_beta_2_shifted =
      q_2_grand_sum_over_h.barycentric_evaluate_at(&worker, beta_2_by_omega)?;

    let polys_for_opening_for_domain_h_at_beta_2 =
      vec![q_2_poly_values_on_h, q_2_grand_sum_over_h.clone()];

    let values_for_opening_for_domain_h_at_beta_2 =
      vec![q_2_eval_at_beta_2, q_2_sum_eval_at_beta_2];
    ...
    let q_1_sum_eval_at_beta_1 =
      proper_q_1_grand_sum_poly_values_over_2h.barycentric_evaluate_at(&worker, beta_1)?;
    let q_1_sum_eval_at_beta_1_shifted =
      proper_q_1_grand_sum_poly_values_over_2h.barycentric_evaluate_at(&worker, beta_1_by_omega)?;
    ...
    // first check f3 wellformedness
    {
      ...
      assert_eq!(
        f_3_well_formedness_baryc_at_beta_3, f_3_at_beta_3,
        "f_3 is reconstructed properly"
      );
