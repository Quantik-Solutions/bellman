...
impl<E: Engine> KeypairAssembly<E> {
  ...
  > {
    ...
    let domain_k_size = *[
      self.num_non_zero_in_a,
      self.num_non_zero_in_b,
      self.num_non_zero_in_c,
    ]
    .iter()
    .max()
    .expect("must exist");
...
pub(crate) fn materialize_domain_elements<F: PrimeField>(
  domain: &Domain<F>,
  worker: &Worker,
) -> Vec<F> {
...
pub(crate) fn eval_unnormalized_bivariate_lagrange_poly_over_different_inputs<F: PrimeField>(
  ...
  let inv_vanishing_at_alpha = vanishing_at_alpha
    .inverse()
    .ok_or(SynthesisError::DivisionByZero)
    .expect("should not vanish on random x");
...
impl<E: Engine> ConstraintSystem<E> for KeypairAssembly<E> {
    ...
    LC: FnOnce(LinearCombination<E>) -> LinearCombination<E>,
  {
    ...
    fn deduplicate_with_sort<E: Engine>(
      ...
      let mut deduped_vec: Vec<(Variable, E::Fr)> = scratch
        .drain()
    ...
    let a = deduplicate_with_sort(
      a(LinearCombination::zero()),
      &mut self.deduplication_scratch,
    );
    let b = deduplicate_with_sort(
      b(LinearCombination::zero()),
      &mut self.deduplication_scratch,
    );
    let c = deduplicate_with_sort(
  ...
  C: Circuit<E>,
{
  ...
  let (domain_h_size, domain_k_size, (a_matrix, b_matrix, c_matrix)) =
    assembly.into_indexer_input(&worker)?;
  ...
  fn interpolate_matrix<F: PrimeField>(
    ...
    let unnormalized_largrange_values_over_k =
      eval_unnormalized_bivariate_lagrange_poly_over_diaginal(domain_h.size, domain_k, &worker);
    ...
    for (row_index, row) in matrix.into_iter().enumerate() {
      for (col_index, coeff) in row {
        ...
        // over K
        let idx_row_into_larger_domain =
          reindex_from_one_domain_to_another_assuming_natural_ordering(
            domain_h, domain_k, row_index,
          );

        let idx_col_into_larger_domain =
          reindex_from_one_domain_to_another_assuming_natural_ordering(
            domain_h, domain_k, col_index,
          );

        let mut lagrange_eval_value =
          unnormalized_largrange_values_over_k[idx_row_into_larger_domain];
        lagrange_eval_value
          .mul_assign(&unnormalized_largrange_values_over_k[idx_col_into_larger_domain]);
  ...
  let (a_num_non_zero, [row_a, col_a, val_a], [row_a_indexes, col_a_indexes]) =
    interpolate_matrix(a_matrix, &domain_h, &domain_k, &worker)?;
  let (b_num_non_zero, [row_b, col_b, val_b], [row_b_indexes, col_b_indexes]) =
    interpolate_matrix(b_matrix, &domain_h, &domain_k, &worker)?;
  let (c_num_non_zero, [row_c, col_c, val_c], [row_c_indexes, col_c_indexes]) =
    interpolate_matrix(c_matrix, &domain_h, &domain_k, &worker)?;
...
pub fn evaluate_bivariate_lagrange_at_diagonal_point<F: PrimeField>(
  x: F,
  vanishing_domain_size: u64,
) -> Result<F, SynthesisError> {
  let mut repr = F::Repr::default();
...
fn evaluate_bivariate_lagrange_at_point_for_vanishing_y<F: PrimeField>(
  x: F,
  y: F,
  vanishing_domain_size: u64,
) -> Result<F, SynthesisError> {
    ...
    for i in 0..params.domain_h_size {
      ...
      for j in 0..params.domain_h_size {
        ...
        for k in 0..params.domain_k_size {
          ...
          let vanishing_at_col_value =
            evaluate_vanishing_for_size(&col_value, params.domain_h_size as u64);
          assert!(vanishing_at_col_value.is_zero());

          let vanishing_at_row_value =
            evaluate_vanishing_for_size(&row_value, params.domain_h_size as u64);
          assert!(vanishing_at_row_value.is_zero());

          let lag_x = evaluate_bivariate_lagrange_at_point_for_vanishing_y(
            x,
            row_value,
            params.domain_h_size as u64,
          )
          .unwrap();
          let lag_y = evaluate_bivariate_lagrange_at_point_for_vanishing_y(
            y,
            col_value,
            params.domain_h_size as u64,
          )
